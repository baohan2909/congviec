...
export function renderTaiKhoan(root) {
  ...
  root.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="row">
        <button class="btn ${theme === 'light' ? 'btn-gold' : 'btn-quiet'}" data-th="light">${ic('sun')} Sáng</button>
        <button class="btn ${theme === 'dark' ? 'btn-gold' : 'btn-quiet'}" data-th="dark">${ic('moon')} Tối</button>
      </div>
  ...
  root.querySelectorAll('[data-th]').forEach((b) => b.onclick = () => {
    ...
    document.querySelector('meta[name=theme-color]')
      ?.setAttribute('content', th === 'dark' ? '#0D1220' : '#F7F4EE');
    renderTaiKhoan(root);
  });
  ...
  $('#tkDoiMk', root).onclick = () => {
    const sh = openSheet(`
      ...
      <button class="btn btn-gold" id="mkOK">${ic('check')} Lưu mật khẩu mới</button>`);
