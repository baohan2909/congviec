...
### v0.2.8 — 09/07/2026 · Nhắc hẹn REO đúng giờ · Giữ mọi công việc · Sửa lỗi ô

**SQL phải chạy:** `UPDATE_v0.2.8.sql`. Bump cache_version → 0.2.8.

**① Nhắc hẹn không rung/reo — TÌM RA GỐC (UPDATE_v0.2.8.sql):** `fn_tao_ke_hoach` trước đây KHÔNG sinh bản ghi nhắc (`nhac_viec`), mà bộ đẩy push (push-sender, chạy mỗi 5 phút) chỉ gửi từ bảng đó — nên kế hoạch không bao giờ được đẩy thông báo. Nay tạo kế hoạch sẽ sinh luôn một bản nhắc đúng mốc (giờ hẹn − nhắc trước); sửa giờ thì cập nhật, hủy thì gỡ nhắc. Push sẽ reo trên iPhone **nếu đã cài PWA (Thêm vào màn hình chính) + cho phép Thông báo**. Ngoài ra mỗi kế hoạch vẫn có nút **"Thêm vào Lịch (báo thức reo)"** để đưa vào Lịch iPhone — cách reo chắc chắn nhất, không phụ thuộc push.

**② Bỏ check hoàn thành — GIỮ MỌI CÔNG VIỆC (12-tab-kehoach.js, 10-tab-homnay.js, UPDATE_v0.2.8.sql):** bỏ nút "Đã xong" (check là mất việc). Kế hoạch giữ mọi mục (chờ + đã làm), chỉ ẩn mục đã hủy — để báo cáo cuối ngày đầy đủ, không mất dữ liệu. `fn_lay_hom_nay` lấy kế hoạch trạng thái ≠ đã hủy.

**③ Cây bút tab Hôm nay — thêm XÓA (10-tab-homnay.js, UPDATE_v0.2.8.sql):** mốc "bắt đầu" (chấm công) giờ có nút **"Xóa chấm công hôm nay"** (`fn_xoa_checkin`); mốc kế hoạch đã có sửa + xóa từ trước.

**④ Form kế hoạch — ô Thời gian tràn (12-tab-kehoach.js):** ô Thời gian (có cả ngày) và ô Nhắc trước trước đây chen cùng một hàng nên tràn/đè. Nay mỗi ô **một hàng riêng** — Thời gian full width ở trên, Nhắc trước ở dưới.

---

### v0.2.7 — 09/07/2026 · Timeline nơi làm việc cân đối · FIX GỐC lỗi đè ô

**SQL:** `UPDATE_v0.2.7.sql` (đồng bộ cache_version → 0.2.7). Thuần frontend.

**① Timeline nơi làm việc — cân đối, mỗi dòng có bút (10-tab-homnay.js, app.css):**
- Bỏ nhãn "bắt đầu"/"hiện tại"; mốc hiện tại nay đánh dấu bằng chấm sáng + địa điểm in màu.
- MỌI dòng đều có icon bút (kể cả mốc đầu) → giờ–chấm–địa điểm–bút thẳng hàng, hết lệch. Bút mốc đầu = đổi loại/nơi làm việc chính; bút mốc kế hoạch = sửa giờ/địa điểm + **xóa mốc**.
- Căn giữa dọc các cột để thẳng hàng.

**② FIX GỐC lỗi đè ô (app.css) — nguồn của "lỗi bị hoài":**
- Rất nhiều form dùng `.row` + `.field flex` mà thiếu `min-width:0`, khiến input giờ/ngày/select (rộng nội tại) tràn đè ô bên cạnh. Nay thêm `.row > .field { min-width: 0 }` và `.row .input { min-width: 0 }` **toàn cục** → mọi form (thêm/sửa nơi làm việc, tạo tài khoản, giao việc, kế hoạch…) hết đè trong một lần sửa.
- Form thêm/sửa nơi làm việc chuyển sang **grid 2 cột** (Giờ cố định + Nơi làm việc co giãn) cho chắc chắn không tràn. Giờ mặc định = giờ hiện tại (sửa được).

**③ Tính năng CẬP NHẬT PHIÊN BẢN (sw.js, 99-app.js, 13-tab-taikhoan.js, app.css):**
- Service worker không còn tự áp bản mới đột ngột. Khi có bản mới, app hiện **banner "Đã có bản mới — Cập nhật"**; bấm là nạp bản mới ngay (hết cảnh phải gỡ/mở lại app nhiều lần).
- Tự kiểm tra cập nhật mỗi khi mở lại app (visibilitychange).
- Tab Tài khoản: cạnh số phiên bản có nút **"Kiểm tra cập nhật"** để chủ động rà.

---

### v0.2.6 — 09/07/2026 · Timeline nơi làm việc (kiểu lịch) · Form thủ công gọn · Dọn lỗi vận hành

**SQL:** `UPDATE_v0.2.6.sql` (đồng bộ cache_version → 0.2.6). Thuần frontend.

**① Camera — xác nhận đã bỏ vòng quay (v0.2.5):** code hiện tại KHÔNG còn animation quay nào trên camera (chỉ đường quét + góc ngắm bằng transform/opacity). Nếu còn thấy vòng quay là do service worker giữ bản cũ — cần deploy bản này + đóng mở lại app.

**② Timeline nơi làm việc kiểu lịch (10-tab-homnay.js, app.css):** đổi sang dạng dòng thời gian giống "Lịch trong ngày" — mỗi mốc có chấm + đường nối + thẻ địa điểm, mốc "bắt đầu" và "hiện tại" được đánh dấu. Mỗi mốc (trừ mốc bắt đầu) có icon bút → sửa giờ / địa điểm / **xóa mốc**. Thêm mốc mới bằng 2 cách: nói trợ lý (tự vào kế hoạch) hoặc thêm thủ công. Badge loại nơi làm việc bấm được để đổi loại (thay nút "Đổi" riêng).

**③ Form thêm thủ công gọn (10-tab-homnay.js):** ô Giờ (ngắn) và Nơi làm việc **cùng một hàng** (min-width:0 chống đè), ghi chú xuống hàng dưới. Sửa lỗi giao diện đè ô ở Hình 3.

**④ Dọn lỗi vận hành:** form thêm nơi làm việc trước đây tạo cả bản ghi di chuyển THỪA — nay chỉ tạo một mốc trong kế hoạch (timeline lấy từ kế hoạch, không trùng dữ liệu). Rà toàn bộ: JS sạch, RPC client↔SQL khớp đủ, ai-gateway cân bằng.

---

### v0.2.5 — 09/07/2026 · Nhận diện nhanh · Hiệu ứng quét sinh trắc

**SQL:** `UPDATE_v0.2.5.sql` (đồng bộ cache_version → 0.2.5). Thuần frontend.

**① Nhận diện & đăng nhập NHANH (07-face.js):**
- Chỉ cần bắt 1 khung tốt là đăng nhập ngay (trước đây bắt buộc 2 khung).
- Nhịp quét dày hơn (260ms → 110ms) để bắt khuôn mặt nhanh.
- Sau khi nhận diện thành công, vào app gần như tức thì (chờ 700ms → 200ms; hiệu ứng thành công rút còn .45s).
- Chờ video ổn định ngắn lại (300ms → 120ms).

**② Đổi hiệu ứng — quét sinh trắc thay vòng xoay (07-face.js, app.css):**
- Bỏ hoàn toàn vòng tròn xoay (nguồn gốc cảm giác khựng khi luồng chính bận).
- Thay bằng **đường quét chạy dọc** kiểu Face ID + **4 góc ngắm** phát sáng nhấp nháy + lưới điểm mờ + viền glow. Tất cả dùng `transform`/`opacity` (chạy trên compositor) nên **mượt tuyệt đối, không khựng** kể cả khi máy đang bận nhận diện.
- Thành công: tắt quét, viền xanh vẽ đầy nhanh + 4 góc chuyển xanh + dấu tích. Thất bại: viền + góc đỏ, rung nhẹ.

---

### v0.2.4 — 09/07/2026 · Vòng quét dứt điểm · Timeline nơi làm việc luôn hiển thị

**SQL:** `UPDATE_v0.2.4.sql` (chỉ đồng bộ cache_version → 0.2.4). Thuần frontend.

**① Vòng quét còn hiện "1/4 vòng" & giật (07-face.js, app.css):** nguyên nhân — overlay mở KHÔNG có trạng thái ban đầu nên dùng CSS base (vòng rỗng) + base có `transition: stroke-dashoffset .4s` → khi vào trạng thái đầu nó VẼ DẦN từ rỗng = "1/4 vòng chạy trước". Đã: (a) đặt trạng thái `dang-tai` ngay khi tạo overlay → spinner mượt từ frame đầu; (b) bỏ transition dashoffset ở base (không còn vẽ 1/4); (c) `will-change: transform` để vòng xoay trên compositor — mượt kể cả khi luồng chính đang bận khởi động WebGL; (d) hạ `inputSize` nhận diện 320→224 để mỗi lần detect nhẹ hơn, camera bớt giật.

**② Timeline nơi làm việc chưa thấy (10-tab-homnay.js):** timeline chỉ hiện khi có kế hoạch CÓ ĐỊA ĐIỂM — nếu chưa có thì trống. Nay LUÔN có **mốc check-in gốc làm điểm bắt đầu** (giờ chấm + nơi làm việc chính, gắn nhãn "bắt đầu"), sau đó là các mốc kế hoạch có địa điểm (mỗi mốc có icon bút sửa giờ + địa điểm). Điều kiện: đã check-in nơi làm việc thì thấy ngay.

---

### v0.2.3 — 09/07/2026 · Camera mượt · Chuyển tab nhanh · Nơi làm việc timeline · Admin quản lý nhân sự

**SQL phải chạy:** `UPDATE_v0.2.3.sql` (RPC admin nhân sự) — và `UPDATE_v0.2.2.sql` nếu chưa chạy. Bump cache_version → 0.2.3.

**① Camera khựng/giật giây đầu (07-face.js):** đảo thứ tự — mở camera TRƯỚC cho hình hiện ngay, chờ video ổn định vài frame rồi mới quét; thêm WARMUP mô hình (chạy 1 inference trên canvas trống lúc tải) để lần detect thật đầu tiên không cold-start làm khựng.

**② Chuyển tab chậm (99-app.js):** trước đây mỗi lần chuyển tab render lại + gọi RPC từ đầu. Nay mỗi tab giữ DOM riêng — chuyển tab là hiện lại tức thì, chỉ tải lại nền nếu dữ liệu cũ >30s.

**③ Nút "Kế hoạch ngay" (10-tab-homnay.js):** tab Hôm nay thêm nút Kế hoạch ngay (khi chưa có) / Lập thêm kế hoạch — thống nhất với Báo cáo ngay.

**④ Nơi làm việc hiện tại — timeline (10-tab-homnay.js, app.css):** hiển thị dạng timeline theo kế hoạch (mỗi mốc: giờ + địa điểm, đánh dấu "hiện tại"). Mỗi dòng có icon bút để sửa giờ + địa điểm (cập nhật thẳng vào kế hoạch). Bỏ nút "Đổi nơi làm việc"; nút "Thêm nơi làm việc" trải ngang phía dưới.

**⑤ Admin quản lý nhân sự đầy đủ (UPDATE_v0.2.3.sql, 13-tab-taikhoan.js):** nút "Danh sách thành viên" mở trang quản lý — xem toàn bộ nhân sự (tìm kiếm), sửa thông tin (họ tên, chức vụ, SĐT, vai trò, phòng ban), **đặt lại mật khẩu** (server cấp, buộc đổi lần đầu), **khóa/mở tài khoản**. RPC: `fn_admin_ds_nhan_su`, `fn_admin_sua_nhan_su`, `fn_admin_dat_mat_khau`, `fn_admin_khoa_mo`.

**⑥ Lịch trình Ban điều hành — sắp theo hoạt động (14-tab-quantri.js, app.css):** người ĐANG HOẠT ĐỘNG (đã chấm địa điểm HOẶC có kế hoạch) xếp lên trên; ô + dấu tròn tô màu gradient hệ thống. Người chưa hoạt động xếp dưới, màu xám/vàng. Hiển thị thêm số kế hoạch mỗi người.

---

### v0.2.2 — 09/07/2026 · Nút Trợ lý · AI viết đầy đủ · Kế hoạch chi tiết · Nơi làm việc hiện tại

**SQL phải chạy:** `UPDATE_v0.2.2.sql` (bảng `ke_hoach_ngay` lưu bản văn bản chi tiết + `fn_luu_ke_hoach_ngay`, `fn_ke_hoach_ngay`, bump cache_version → 0.2.2).
**Edge Function deploy lại:** `ai-gateway` (prompt viết đầy đủ, tự nhiên hơn).

**① Nút "Trợ lý — nói để báo cáo" không phản hồi (04-voice.js):** biến ghi âm `ctx` kẹt lại từ lần trước (overlay đã rời DOM) → `if (ctx) return` chặn im lặng mọi lần bấm sau. Nay reset ctx kẹt + báo lỗi mic rõ ràng (quyền micro) thay vì im lặng.

**② Nút AI nổi bật (11-tab-baocao.js, app.css):** nút "Trợ lý chuẩn hóa & gửi" đổi thành nút gradient nổi bật (class `.btn-troly`) giống nút Trợ lý chính — các tính năng AI dễ nhận biết hơn.

**③ Dạng Văn bản dư 2 dấu chấm (app.css):** `.md-ul` thiếu `list-style: none` → vừa bullet mặc định của `<ul>` (chấm ngoài) vừa `::before` (chấm trong). Nay chỉ còn 1 chấm.

**④ AI viết đầy đủ, tự nhiên hơn (ai-gateway):** prompt báo cáo & kế hoạch đổi hướng — giữ 2 nhóm 1 cấp bullet, nhưng mỗi mục viết thành CÂU HOÀN CHỈNH, diễn đạt mạch lạc như người quản lý viết (hiểu ý rồi chuẩn hóa), không cụt kiểu từ khóa.

**⑤ Kế hoạch chi tiết — lưu bản đầy đủ (UPDATE_v0.2.2.sql, 05-troly.js, 12-tab-kehoach.js):** trước đây bản văn bản AI viết KHÔNG được lưu, mở lại chỉ build ngắn từ list. Nay lưu vào bảng `ke_hoach_ngay`; dạng Văn bản của tab Kế hoạch hiển thị bản chi tiết đầy đủ (copy sang Sheets / in ấn).

**⑥ Vòng quét khuôn mặt (app.css):** đổi cung 28% (nhìn như "đoạn xoay xoay") thành cung ~90% quay đều — nhìn như vòng tròn liền mạch, chỉ một khe nhỏ chạy quanh.

**⑦ Thêm nơi làm việc bị đè UI (10-tab-homnay.js):** input `type=time` (iOS hiện AM/PM) đẩy rộng, tràn đè ô bên cạnh trong flexbox. Nay xếp mỗi ô một dòng riêng.

**⑧ "Nơi làm việc hôm nay" → "hiện tại" (10-tab-homnay.js, app.css):** đổi tên; thêm đồng hồ giờ chạy; nơi làm việc hiện tại suy từ mốc di chuyển có giờ ≤ hiện tại (đánh dấu "hiện tại"); khi thêm nơi làm việc thì tạo luôn một mục trong Kế hoạch đúng giờ đó.
